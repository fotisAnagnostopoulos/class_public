...
// CORRECT line to replace it with
static double adaptive_simpson_recursive(struct background *pba, double a, double b, double epsilon, double whole, int recursion_depth);

/**
 * This is the function we want to integrate: 3*(1+w)/a.
 * It's safer to define it separately.
 */
static double fld_integrand(struct background *pba, double a) {
    double w = compute_w_de(pba, a);
...
/**
 * Performs one step of Simpson's rule.
 */
static double simpson_step(struct background *pba, double a, double b) {
    double c = (a + b) / 2.0;
    double fa = fld_integrand(pba, a);
    double fb = fld_integrand(pba, b);
    double fc = fld_integrand(pba, c);
    return (b - a) / 6.0 * (fa + 4.0 * fc + fb);
}

/**
 * Recursive core of the adaptive Simpson's method.
 */
static double adaptive_simpson_recursive(struct background *pba, double a, double b, double epsilon, double whole, int recursion_depth) {
    
    /* ==> ADD THIS CHECK AT THE TOP <== */
    // Hard limit to prevent excessive recursion and slowness.
    int max_recursion_depth = 20;
    if (recursion_depth >= max_recursion_depth) {
        // Stop recursing and return the best estimate so far.
        return whole;
    }

    double c = (a + b) / 2.0;
    double left = simpson_step(pba, a, c);
    double right = simpson_step(pba, c, b);

    if (fabs(left + right - whole) <= 15.0 * epsilon) {
        return left + right + (left + right - whole) / 15.0;
    }

    // Pass the incremented depth to the recursive calls
    return adaptive_simpson_recursive(pba, a, c, epsilon / 2.0, left, recursion_depth + 1) +
           adaptive_simpson_recursive(pba, c, b, epsilon / 2.0, right, recursion_depth + 1);
}
/**
...
static double rk45_solver(
    ...
    while (((h > 0 && a < a_end) || (h < 0 && a > a_end)) && (step_count < MAX_STEPS)) {
        ...
        double k3 = h * f(a + 3.0 * h / 8.0, y + 3.0 * k1 / 32.0 + 9.0 * k2 / 32.0, params);
        double k4 = h * f(a + 12.0 * h / 13.0, y + 1932.0 * k1 / 2197.0 - 7200.0 * k2 / 2197.0 + 7296.0 * k3 / 2197.0, params);
        double k5 = h * f(a + h, y + 439.0 * k1 / 216.0 - 8.0 * k2 + 3680.0 * k3 / 513.0 - 845.0 * k4 / 4104.0, params);
        double k6 = h * f(a + h / 2.0, y - 8.0 * k1 / 27.0 + 2.0 * k2 - 3544.0 * k3 / 2565.0 + 1859.0 * k4 / 4104.0 - 11.0 * k5 / 40.0, params);
        ...
        double scale = (error == 0.0) ? 4.0 : SAFETY * pow(tol / error, 0.25);
        h *= fmin(fmax(scale, 0.1), 4.0);
        step_count++;
    }
    return y;
}
...
//         w_i = w_ip1;
//     }
//     return -3.0*result*2.004008016032064;
// }
/**
...
int background_functions(
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    /* ==> ADD THIS LINE TO PRINT THE EVOLUTION OF w(a) <== */
    printf("W_EVOLUTION   %e   %e\n", a, w_fld);
...
int background_initial_conditions(
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    for (counter=0; counter < _MAX_IT_; counter++) {
      ...
      rho_ncdm_rel_tot = 0.;
      for (n_ncdm=0; n_ncdm<pba->N_ncdm; n_ncdm++) {
        class_call(background_ncdm_momenta(pba->q_ncdm_bg[n_ncdm],pba->w_ncdm_bg[n_ncdm],
                                           pba->q_size_ncdm_bg[n_ncdm],pba->M_ncdm[n_ncdm],
                                           pba->factor_ncdm[n_ncdm],1./a-1.0,
                                           NULL,&rho_ncdm,&p_ncdm,NULL,NULL),
                   pba->error_message,pba->error_message);
